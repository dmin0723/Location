package com.dengmin.location.utils.photo;
...
import com.dengmin.location.activity.ActivityCamera;
import com.dengmin.location.activity.ImageDetailsActivity;
...
public class MyScrollView extends ScrollView implements OnTouchListener {
		...
		public void handleMessage(Message msg) {
			...
			if (scrollY == lastScrollY) {
				...
				if (scrollViewHeight + scrollY >= scrollLayout.getHeight() && taskCollection.isEmpty()) {
					myScrollView.loadMoreImages(); //下载图片
				}
				myScrollView.checkVisibility();//设置可见性，不在当前页面就使用空图片
			} else {
				lastScrollY = scrollY;
	...
	class LoadImageTask extends AsyncTask<Integer, Void, Bitmap> {
		...
		/**
		 * 根据传入的URL，对图片进行加载。如果这张图片已经存在于SD卡中，
		 * 则直接从SD卡里读取，否则就从网络上下载。
		 *
		 * @param imagePath 图片的URL地址
		 * @return 加载到内存的图片。
		 */
